...
NIL is the ordinary state of a desktop nobody has dialed into, and every caller here is written so
that NIL costs nothing: no error, no log line, and no microphone."
  (let ((srv *session-mic-stream*))
         ...
         (handler-case
             (progn
               ...
               (let* ((req (%parse-params (or (%read-ascii-line stream) "")))
                 ...
                 ;; the newest microphone is the session's microphone
