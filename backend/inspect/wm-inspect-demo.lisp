;;;; wm-inspect-demo.lisp — the Genera-lineage object inspector (McCLIM's
;;;; Clouseau) as a window on the glass OPEN LOOK desktop.  Opens the inspector
;;;; on a nested Lisp object and drills into a slot by clicking a value.
;;;;   sbcl --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-inspect-demo.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:mcclim :mcclim-render :clouseau :glass :zpng :chipz))
    (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:winsp (:use #:cl)) (in-package #:winsp)
...
;; A nested object to inspect: a CLOS instance whose slots are varied types
;; (list, hash-table, vector, another instance) so the drill-down has somewhere
;; to go.
(defclass cpu () ((name :initform "Ivory" :initarg :name)
                  (bits :initform 40 :initarg :bits)))
(defclass lispm ()
  ((name    :initform "Genera" :initarg :name)
   (year    :initform 1988)
   (authors :initform '("symbolics" "lmi" "ti"))
   (specs   :initform (let ((h (make-hash-table :test 'equal)))
                        (setf (gethash "clock-mhz" h) 36 (gethash "words" h) (expt 2 32)) h))
   (words   :initform #(:tag :ivory 3 1 4 1 5 9))
   (cpu     :initform (make-instance 'cpu))))

(defvar *demo* (make-instance 'lispm))

(let ((port 5959))
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '((:inspect *demo*))         ; open the inspector on *demo*
                                     :port port :width 900 :height 620
                                     :menu '(("Inspect *demo*" :inspect *demo*)
                                             ("Terminal"       :terminal)))
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&inspect desktop: ~dx~d~%" w h)
      (sleep 4.0)                                   ; frame + inspector come up
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-inspect.png")
        (format t "inspector captured~%")
        ;; drill in: left-click the nested CPU object (the CPU slot value) to
        ;; expand it inline and reveal its own slots
        (lclick s 250 299)
        (sleep 1.2)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-inspect-2.png")
        (format t "drilled~%"))
