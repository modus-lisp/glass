;;;; wm-wallpaper-demo.lisp — an SVG desktop background.  weft rasterises a
;;;; bundled wallpaper.svg into a screen-sized framebuffer; the WM composites
;;;; windows on top.  No SDL, no X.
;;;;   sbcl --control-stack-size 256 --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-wallpaper-demo.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:glass :zpng :chipz :weft/render :clim-examples))
    (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:wwp (:use #:cl)) (in-package #:wwp)
...
(let* ((wp (namestring (merge-pathnames "assets/wallpaper.svg" (asdf:system-source-directory :mcclim-glass))))
  ...
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '((:terminal :cols 54 :rows 12 :ppem 13)
                                       (clim-demo.calculator::calculator-app :width 360 :height 320))
                                     :port port :width 1040 :height 720
                                     :background wp :background-mode :cover)
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&wallpaper desktop: ~dx~d~%" w h)
      (sleep 4.0)
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-wallpaper.png")
        (format t "captured~%"))
