;;;; wm-submenu-demo.lisp — the restructured root menu: Browse / Inspect / Debug /
;;;; Terminal + an Apps submenu.  Right-click the workspace, hover "Apps" to open
;;;; the submenu, then launch an app from it.  Runs in-process, so it introspects
;;;; the live menu tree for exact click points.
;;;;   sbcl --control-stack-size 256 --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-submenu-demo.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:glass :zpng :chipz :clim-examples :clim-listener))
    (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:wsub (:use #:cl)) (in-package #:wsub)
...
(defun ptr (s mask x y) (w8 s 5) (w8 s mask) (w16 s x) (w16 s y) (force-output s) (sleep 0.1))
(defun rclick (s x y) (ptr s 4 x y) (ptr s 0 x y) (sleep 0.25))
(defun lclick (s x y) (ptr s 0 x y) (ptr s 1 x y) (ptr s 0 x y) (sleep 0.35))

;; live menu introspection (same image as the WM)
(defun item-point (m idx)                     ; screen center of item IDX in menu M
  (values (+ (clim-glass::wm-menu-x m) (floor (glass:fb-width (clim-glass::wm-menu-fb m)) 2))
          (+ (clim-glass::wm-menu-y m) 20 (* idx 20) 10)))          ; titleh=20, itemh=20
(defun item-index (m label)
  (position label (clim-glass::wm-menu-items m) :key #'car :test #'string=))

(let ((port 5975))
  (sb-thread:make-thread
   (lambda () (handler-case (clim-glass:run-wm '() :port port :width 1040 :height 720)
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)) (p nil))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&submenu desktop: ~dx~d~%" w h)
      (sleep 2.0)
      (setf p (clim-glass::find-glass-port :port port))
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (rclick s 320 150)                        ; open the root menu
        (let* ((root (clim-glass::glass-port-menu p))
               (ai (item-index root "Apps")))
          (multiple-value-bind (ax ay) (item-point root ai)
            (ptr s 0 ax ay) (sleep 0.3))          ; hover Apps -> submenu opens
          (save-png (read-frame s w h dstate) w h "/tmp/glass-submenu.png")
          (format t "root+submenu captured (root items: ~a)~%"
                  (mapcar #'car (clim-glass::wm-menu-items root)))
          ;; click "Tabbed Terminal" in the submenu
          (let ((sub (clim-glass::wm-menu-child root)))
            (format t "submenu items: ~a~%" (mapcar #'car (clim-glass::wm-menu-items sub)))
            (multiple-value-bind (tx ty) (item-point sub (item-index sub "Tabbed Terminal"))
              (lclick s tx ty)))
          (sleep 2.5)
          (save-png (read-frame s w h dstate) w h "/tmp/glass-submenu-launched.png")
          (format t "launched from submenu~%")))
