;;;; wm-browse-demo.lisp — a loom/weft BROWSER window on the glass OPEN LOOK
;;;; desktop, launched FROM THE ROOT MENU.  Right-click the workspace, pick
;;;; "Browse", and loom renders weft into a decorated window; then click a link
;;;; inside that window to navigate in place.  No SDL, no X.
;;;;   sbcl --control-stack-size 256 --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-browse-demo.lisp
...
(defun rclick (s x y) (ptr s 4 x y) (ptr s 0 x y) (sleep 0.25))         ; right press+release
(defun lclick (s x y) (ptr s 0 x y) (ptr s 1 x y) (ptr s 0 x y) (sleep 0.3))

(let* ((home (namestring (merge-pathnames "assets/home.html" (asdf:system-source-directory :loom))))
       (port 5973))
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '() :port port :width 1040 :height 720
                                     :menu (list (list "Browse loom home" :browse home :width 900 :height 600)
                                                 (list "Terminal" :terminal)))
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&browse desktop: ~dx~d~%" w h)
      (sleep 2.0)
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (rclick s 300 200)                             ; right-click workspace -> root menu
        (ptr s 0 340 220) (sleep 0.2)                  ; hover "Browse loom home"
        (save-png (read-frame s w h dstate) w h "/tmp/glass-browse-menu.png")
        (format t "menu captured~%")
        (lclick s 340 220)                             ; click Browse -> loom window opens
        (sleep 3.0)                                     ; fetch + render weft
        (save-png (read-frame s w h dstate) w h "/tmp/glass-browse.png")
        (format t "browser captured~%")
        ;; the window content top-left is at (40, 40+titleh); the "about page"
        ;; link hit-tests near page (206,316) -> screen ~ (246, 378)
        (lclick s 250 380)
        (sleep 2.0)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-browse-nav.png")
        (format t "navigated~%"))
