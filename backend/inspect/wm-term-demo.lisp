;;;; wm-term-demo.lisp — a terminal AND a CLIM app as windows on the OPEN LOOK
;;;; desktop; type into the (focused) terminal window over VNC.
;;;;   sbcl --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-term-demo.lisp
...
(let ((port 5951))
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '((clim-demo::gadget-test :width 360 :height 300)
                                       (:terminal :cols 66 :rows 18 :ppem 13))
                                     :port port :width 900 :height 620)
                (error (e) (format t "~&WM ERROR ~a~%" e))))
   :name "wm")
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&wm+term desktop: ~dx~d~%" w h)
      (sleep 3.5)                                            ; frame + terminal shell come up
      ;; the terminal was added last, so it has keyboard focus — type into it
      (typ s "echo hi from the wm terminal") (key s #xff0d) (sleep 0.4)
      (typ s "ls --color=always -d /etc /usr /bin; echo emoji works too") (key s #xff0d) (sleep 0.8)
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-wm-term.png")
        (format t "captured /tmp/glass-wm-term.png~%"))
