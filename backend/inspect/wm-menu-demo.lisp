;;;; wm-menu-demo.lisp — the OPEN LOOK workspace root menu: right-click the bare
;;;; teal desktop to pop up the "Workspace" menu, hover an item, launch a terminal.
;;;;   sbcl --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-menu-demo.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:glass :zpng :chipz)) (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:wmenu (:use #:cl)) (in-package #:wmenu)
...
;; pointer: send an RFB PointerEvent (button MASK) at (x,y)
(defun ptr (s mask x y) (w8 s 5) (w8 s mask) (w16 s x) (w16 s y) (force-output s) (sleep 0.05))
(defun rclick (s x y) (ptr s 4 x y) (ptr s 0 x y) (sleep 0.2))     ; right press+release
(defun lclick (s x y) (ptr s 1 x y) (ptr s 0 x y) (sleep 0.3))     ; left press+release
...
(let ((port 5957))
  ...
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&menu desktop: ~dx~d~%" w h)
      (sleep 1.5)
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (rclick s 210 150)                          ; right-click bare workspace -> menu at (210,150)
        (ptr s 0 250 180) (sleep 0.2)               ; hover item 0 ("Terminal") -> highlight
        (save-png (read-frame s w h dstate) w h "/tmp/glass-menu.png")
        (format t "menu captured~%")
        (lclick s 250 180)                          ; click "Terminal" to launch
        (sleep 2.5)
        (typ s "echo launched from the workspace menu") (key s #xff0d) (sleep 0.5)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-menu-term.png")
        (format t "launched~%"))
