;;;; wm-demo.lisp — the OPEN LOOK window manager: two decorated windows on the
;;;; teal workspace, then drag one by its title bar.  Screenshots along the way.
;;;;   sbcl --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-demo.lisp
...
(in-package #:glass-wm-demo)

;; a small second window so the desktop has some variety
(clim:define-application-frame hello ()
  ()
  (:menu-bar nil)
  (:panes (canvas :application :display-function 'draw-hello :scroll-bars nil :width 300 :height 170))
  (:layouts (default canvas)))
(defun draw-hello (frame pane)
  (declare (ignore frame))
  (clim:draw-rectangle* pane 0 0 300 170 :ink (clim:make-rgb-color 0.96 0.96 0.90))
  (clim:draw-text* pane "glass wm" 24 46 :text-size 30 :ink clim:+black+)
  (clim:draw-text* pane "OPEN LOOK, over VNC" 24 86 :text-size 15 :ink (clim:make-rgb-color 0.15 0.30 0.60))
  (clim:draw-text* pane "drag me by the title bar" 24 120 :text-size 12 :ink (clim:make-gray-color 0.4)))
...
(let ((port 5945))
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '((clim-demo::gadget-test :width 460 :height 380)
                                       (hello :width 300 :height 170))
                                     :port port :width 900 :height 640)
                (error (e) (format t "~&WM ERROR ~a~%" e) (finish-output))))
   :name "wm")
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&wm desktop: ~dx~d~%" w h)
      (sleep 4.0)                                            ; let both windows come up
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-wm-0.png")
        (format t "workspace captured~%")
        ;; drag the top window (hello) by its title bar: cascade puts it at ~(68,68)
        (let ((tx 180) (ty 78))                              ; a point on hello's title bar
          (ptr s 0 tx ty) (ptr s 1 tx ty)                    ; move, press
          (loop for k from 1 to 8 do (ptr s 1 (+ tx (* k 18)) (+ ty (* k 14))))  ; drag
          (ptr s 0 (+ tx 144) (+ ty 112)))                   ; release
        (sleep 0.8)
        (let ((cli (read-frame s w h dstate)))
          (save-png cli w h "/tmp/glass-wm-1.png")
          (format t "after title-bar drag: saved /tmp/glass-wm-1.png~%")))
