;;;; wm-calc-demo.lisp — launch McCLIM's Calculator from the Apps submenu, then
;;;; press a few keys and read the result, all over VNC.
;;;;   sbcl --control-stack-size 256 --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-calc-demo.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:glass :zpng :chipz :clim-examples))
    (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:wcalc (:use #:cl)) (in-package #:wcalc)
...
(let ((port 5977))
  ...
  (let ((s (connect port)) (p nil))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&calc desktop: ~dx~d~%" w h)
      (sleep 2.0)
      (setf p (clim-glass::find-glass-port :port port))
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (rclick s 320 150)
        (let ((root (clim-glass::glass-port-menu p)))
          (multiple-value-bind (ax ay) (item-point root (item-index root "Apps"))
            (ptr s 0 ax ay) (sleep 0.3))                  ; hover Apps
          (let ((sub (clim-glass::wm-menu-child root)))
            (multiple-value-bind (tx ty) (item-point sub (item-index sub "Calculator"))
              (lclick s tx ty))))                          ; click Calculator
        (sleep 2.5)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-calc.png")
        (format t "calculator launched~%")
        ;; press digits 4 then 2 (grid: left col x~113, right col x~267;
        ;; rows 1/2 y~201, 3/4 y~241) — display should read 42
        (lclick s 267 241)          ; 4
        (lclick s 267 201)          ; 2
        (sleep 0.5)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-calc-42.png")
        (format t "typed 42~%"))
