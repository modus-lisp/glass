;;;; damage-visual.lisp — correctness check for damage-clipped compositing: two
;;;; terminals over a wallpaper; type in each, drag one ACROSS the other and away.
;;;; If the clip is right, both terminals + wallpaper stay intact (no stale pixels).
;;;;   sbcl --control-stack-size 256 --dynamic-space-size 4096 --non-interactive --load backend/inspect/damage-visual.lisp

(require :asdf)
(load "~/quicklisp/setup.lisp")
(handler-bind ((warning #'muffle-warning))
  (let ((*standard-output* (make-broadcast-stream)))
    (ql:quickload '(:glass :glass/term :weft/render :zpng :chipz))
    (require :sb-concurrency)
    (asdf:load-asd "/home/claude/glass/backend/mcclim-glass.asd")
    (asdf:load-system :mcclim-glass)))

(defpackage #:dv (:use #:cl)) (in-package #:dv)
...
(defun drag (s x0 y0 x1 y1)
  (ptr s 0 x0 y0)(ptr s 1 x0 y0)
  (dotimes (i 12) (ptr s 1 (round (+ x0 (* (- x1 x0) (/ (1+ i) 12)))) (round (+ y0 (* (- y1 y0) (/ (1+ i) 12))))))
  (ptr s 0 x1 y1)(sleep 0.3))
(defun click (s x y) (ptr s 0 x y)(ptr s 1 x y)(ptr s 0 x y)(sleep 0.2))

(let* ((wp (namestring (merge-pathnames "assets/wallpaper.svg" (asdf:system-source-directory :mcclim-glass))))
       (port 5991))
  (sb-thread:make-thread
   (lambda () (handler-case
                  (clim-glass:run-wm '((:terminal :cols 40 :rows 10 :ppem 14)
                                       (:terminal :cols 40 :rows 10 :ppem 14))
                                     :port port :width 1100 :height 720 :background wp)
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)) (p nil))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&damage-visual: ~dx~d~%" w h)
      (sleep 3.0)
      (setf p (clim-glass::find-glass-port :port port))
      (let ((surfs (clim-glass::glass-port-surfaces p)) (dstate (chipz:make-dstate 'chipz:zlib)))
        ;; surfaces are newest-first: (second) is terminal 1, (first) is terminal 2
        (let ((t1 (second surfs)) (t2 (first surfs)))
          ;; type into terminal 2 (focused)
          (typ s "echo TTT-TWO-window") (key s #xff0d) (sleep 0.3)
          ;; focus + type into terminal 1 (click its content)
          (click s (+ (clim-glass::wm-surface-x t1) 60) (+ (clim-glass::wm-surface-y t1) 40))
          (typ s "echo TTT-ONE-window") (key s #xff0d) (sleep 0.3)
          ;; drag terminal 1 ACROSS terminal 2 and beyond, then back
          (let ((tx (+ (clim-glass::wm-surface-x t1) 100)) (ty (- (clim-glass::wm-surface-y t1) 11)))
            (drag s tx ty (+ tx 420) (+ ty 220))
            (drag s (+ tx 420) (+ ty 220) tx ty)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-damage-visual.png")
        (format t "captured~%"))
