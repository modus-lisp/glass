;;;; wm-tabs-demo.lisp — a tabbed terminal on the WM desktop: type in shell 1,
;;;; open a new tab (+), type in shell 2, switch back.
;;;;   sbcl --dynamic-space-size 4096 --non-interactive --load backend/inspect/wm-tabs-demo.lisp
...
(defpackage #:wtabs (:use #:cl)) (in-package #:wtabs)
...
(defun click (s x y) (w8 s 5) (w8 s 0) (w16 s x) (w16 s y) (force-output s) (sleep 0.05)
  (w8 s 5) (w8 s 1) (w16 s x) (w16 s y) (force-output s) (sleep 0.05)
  (w8 s 5) (w8 s 0) (w16 s x) (w16 s y) (force-output s) (sleep 0.3))

(let ((port 5955))
  (sb-thread:make-thread
   (lambda () (handler-case (clim-glass:run-wm '((:tabterm :cols 70 :rows 16 :ppem 13))
                                               :port port :width 760 :height 480)
                (error (e) (format t "~&WM ERROR ~a~%" e)))))
  (let ((s (connect port)))
    (multiple-value-bind (w h) (handshake s)
      (format t "~&tabs desktop: ~dx~d~%" w h)
      (sleep 2.5)
      (typ s "echo THIS IS SHELL ONE") (key s #xff0d) (sleep 0.4)
      (click s 205 73)                       ; "+" new tab (tab0 0-130, + at ~130-260)
      (sleep 0.8)
      (typ s "echo now in the SECOND tab") (key s #xff0d) (sleep 0.4)
      (let ((dstate (chipz:make-dstate 'chipz:zlib)))
        (save-png (read-frame s w h dstate) w h "/tmp/glass-tabs-2.png")
        (click s 100 73)                     ; click "sh 1" tab -> switch back
        (sleep 0.6)
        (save-png (read-frame s w h dstate) w h "/tmp/glass-tabs-1.png"))
      (format t "captured~%")
